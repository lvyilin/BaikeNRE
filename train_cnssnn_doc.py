...
CWD = os.getcwd()
SAVE_MODEL_PATH = os.path.join(CWD, "net_params", "cnssnn_doc", "net_cnssnn_doc_epoch%d_12610.params")
WORD_DIMENSION = 100
POS_DIMENSION = 5
...
CTX = mx.cpu(0)
ctx = [CTX]
fail_id_file = open("fail_id_cnssnn_doc.txt", "w")

input_train = np.load('data_train_cnssnn_id_doc.npy')
input_test = np.load('data_test_cnssnn_id_doc.npy')
...
def evaluate_model(net, epoch):
    ...
    np.save("result_cnssnn_doc.npy", result.asnumpy())
...
class Network(nn.Block):
    ...
    def forward(self, input_data):
        docvec = input_data[:, 0:2 * DOC_LENGTH]
        e1_vec_start = FIXED_WORD_LENGTH * DIMENSION + 2 * DOC_LENGTH
        x = input_data[:, 2 * DOC_LENGTH:e1_vec_start].reshape(
        ...
        out = self.output
        y4 = nd.concat(y1, center_y, docvec, dim=1)
        y5 = out(y4)
        return y5

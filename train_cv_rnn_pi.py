...
input_train = np.load('data_train_rnn_pi_p.npy'.format(MARK))
input_test = np.load('data_test_rnn_pi_p.npy'.format(MARK))
...
class Network(nn.Block):
    ...
    def forward(self, input_data):
        h = self.lstm(input_data[2:].reshape((input_data.shape[0], FIXED_WORD_LENGTH, DIMENSION)))
        return self.output(h.expand_dims(1))

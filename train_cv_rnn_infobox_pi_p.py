...
class Network(nn.Block):
    def __init__(self, prefix=None, params=None):
        ...
        with self.name_scope():
            ...
            #             self.att.add(nn.Dense(1, flatten=False,activation="tanh"))
            ...
            self.output1.add(nn.Flatten())
            self.output1.add(nn.Activation(activation='relu'))
            self.output1.add(nn.Dropout(0.5))
            self.output1.add(nn.Dense(7))
            self.output2 = nn.Sequential()
            self.output2.add(nn.Flatten())
            self.output2.add(nn.Activation(activation='relu'))
            self.output2.add(nn.Dropout(0.5))
            self.output2.add(nn.Dense(7))
    ...
    def forward(self, input_data):
        ...
        #         (batch_size,256)
        h_sen_infobox = nd.concat(h_sen_new.reshape((h_sen_new.shape[0], -1)), e_infobox_list_all_att,
                                  dim=1)  # (128, 384) (128, 384)
        y1 = self.output1(y_out)
        y2 = self.output2(h_sen_infobox)
        return y1 + y2

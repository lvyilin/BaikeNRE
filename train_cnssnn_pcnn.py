...
CWD = os.getcwd()
SAVE_MODEL_PATH = os.path.join(CWD, "net_params", "cnssnn_pcnn", "net_cnssnn_pcnn_epoch%d_12610.params")
WORD_DIMENSION = 100
POS_DIMENSION = 5
...
CTX = mx.cpu(0)
ctx = [CTX]
fail_id_file = open("fail_id_cnssnn_pcnn.txt", "w")
...
def evaluate_model(net, epoch):
    ...
    np.save("result_cnssnn_pcnn.npy", result.asnumpy())

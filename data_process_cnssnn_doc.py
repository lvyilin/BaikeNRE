...
DOCVEC = os.path.join(CWD, "docvectors.kv")
...
def get_doc_vec(entity_name):
    if entity_name in docvec:
        return docvec[entity_name]
    else:
        return np.zeros(docvec["毛泽东"].shape)
...
docvec = KeyedVectors.load(DOCVEC, mmap='r')

PLACEHOLDER = np.zeros(DIMENSION)
POS_VECTOR = np.random.random((FIXED_WORD_LENGTH * 2, POS_DIMENSION))

for corpus, save_filename in ((CORPUS_TRAIN, "data_train_cnssnn_id_doc.npy"),
                              (CORPUS_TEST, "data_test_cnssnn_id_doc.npy")):
    output_idx = []
    output_entity_pos = []
    output_relative_pos = []
    output_sentence = []
    ...
    with open(corpus, "r", encoding="utf8") as f:
        for line in f:
            ...
            output_en1_doc.append(get_doc_vec(entity_a))
            output_en2_doc.append(get_doc_vec(entity_b))
    ...
    np_en1_doc = np.array(output_en1_doc, dtype=float)
    np_en2_doc = np.array(output_en2_doc, dtype=float)
    ...
    print(np_en1_doc.shape)
    print(np_en2_doc.shape)

    np_entity_edge_vec = np.concatenate((np_en1_vec, np_en2_vec), axis=1)
    np_entity_doc = np.concatenate((np_en1_doc,
                                    np_en2_doc), axis=1)
    print(np_entity_doc.shape)
